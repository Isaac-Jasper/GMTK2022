...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void VerticalMovement()
    {
        ...
        {
            SFXControlleer.sfx.PlaySound("MarioJump");
    ...
    private bool Grounded()
    {
        ...
        RaycastHit2D[] hit = Physics2D.BoxCastAll(transform.position - new Vector3(0,transform.localScale.y/2,0),new Vector2(transform.localScale.x-0.01f,0.05f),0,Vector2.down,1, groundLayer); //ISAAC ADDITION - added transform.localscale.x so the size of the isgrounded check scales linearly with cube size
